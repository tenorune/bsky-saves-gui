...
describe('buildStatusPayload', () => {
  ...
  // Coord-doc Q13: the helper rejects payloads with `library.total_saves:
  // null` AND payloads with no `library` block at all, so during cold-
  // start (inventoryState !== 'ready') we emit a placeholder block with
  // total_saves: 0 + by_status zeros. The panel reads `current_state`
  // for the in-flight signal; the zeroed counts are placeholder values
  // until the fetch completes and the real inventory lands.
  it('emits library with total_saves: 0 + by_status zeros when inventoryState is loading', () => {
    const payload = buildStatusPayload({
      ...BASE_INPUTS,
    ...
    expect(payload!.library.by_status).toEqual({ synced: 0, lost: 0, unsaved: 0 });
  });

  it('emits library with total_saves: 0 + by_status zeros when inventoryState is empty', () => {
    const payload = buildStatusPayload({
      ...BASE_INPUTS,
  ...
  it('emits library with total_saves: 0 + by_status zeros when inventoryState has errored', () => {
    const payload = buildStatusPayload({
      ...BASE_INPUTS,
  ...
  it('emits library with total_saves: 0 when inventory is ready and the user has zero saves', () => {
    const payload = buildStatusPayload({
      ...BASE_INPUTS,
      inventoryState: { status: 'ready', inventory: { saves: [] } as never },
    });
